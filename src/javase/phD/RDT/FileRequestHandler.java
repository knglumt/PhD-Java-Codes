		...
		if (file==null){
			...
			try {
				...
				while(tmpEndByte<endByte){
					...
					//Policy 5000
					if (serverSocket.getLocalPort()==5000) {
						
						if ((tmpStartByte > 7000) && (tmpStartByte < 9000)) {
							try {
						...
						if (!((tmpStartByte > 1000) && (tmpStartByte < 3000))) {
					...
					if ((serverSocket.getLocalPort()==5001)
						|| (serverSocket.getLocalPort()==5002)) {
						
						if ((tmpStartByte > 10000) && (tmpStartByte < 12000)) {
							try {
								TimeUnit.MILLISECONDS.sleep(99);
							} catch (InterruptedException e) {
								// TODO Auto-generated catch block
						...
						if (!((tmpStartByte > 4000) && (tmpStartByte < 6000))) {
							sendBytes(dataResponsePackage.toByteArray());
						}
					}
				...
				System.out.println("Transferred fileId:"+file_id+" file_name:"+file.getName()+" size:"+(endByte-request.getStart_byte()+1));
