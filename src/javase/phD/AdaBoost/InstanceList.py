...
class InstanceList(object):
    ...
    def attributeClassDistributionWeighted(self, classWeights:list, attributeIndex: int) -> list:
        """
        The attributeClassDistribution method takes an attribute index as an input. It loops through the instances, gets
        the corresponding value of given attribute index and adds the class label of that instance to the discrete
        distributions list and class weights.

        PARAMETERS
        ----------
        classWeights : list
            List of the class weights.
        attributeIndex : int
            Index of the attribute.
        ...
        """
        distributions = []
        distributionsWeights = []
        valueList = self.getAttributeValueList(attributeIndex)
        for _ in valueList:
            distributions.append(DiscreteDistribution())
            distributionsWeights.append(DiscreteDistribution())
        for instance, weight in zip(self.list, classWeights):
            distributions[valueList.index(instance.getAttribute(attributeIndex).getValue())].addItem(instance.getClassLabel())
            distributionsWeights[valueList.index(instance.getAttribute(attributeIndex).getValue())].addItem(weight)
        return distributions, distributionsWeights
    ...
    def discreteIndexedAttributeClassDistributionWeighted(self, classWeights: list, attributeIndex: int, attributeValue: int) -> \
            DiscreteDistribution:
        """
        The discreteIndexedAttributeClassDistribution method takes an attribute index and an attribute value as inputs.
        It loops through the instances, gets the corresponding value of given attribute index and given attribute value.
        Then, adds the class label and the class weights of that instance to the discrete indexed distributions list and class weights.
        ...
        DiscreteDistribution
            Distribution of the class labels.
        weights Rate
            Distribution rate of the class weights
        """
        distribution = DiscreteDistribution()
        ...
        return distribution, sum(weights) / sum(classWeights)
